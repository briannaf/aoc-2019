...
def intcode_comp(data, phase, amp_input):
...
def get_thruster_signal(levels, program):
    amp_a_out = intcode_comp(program, levels[0], 0)
    amp_b_out = intcode_comp(program, levels[1], amp_a_out)
    amp_c_out = intcode_comp(program, levels[2], amp_b_out)
    amp_d_out = intcode_comp(program, levels[3], amp_c_out)
    amp_e_out = intcode_comp(program, levels[4], amp_d_out)
    return int(amp_e_out)


data = parse_to_array('day7input.txt')
level_source = [0, 1, 2, 3, 4]
